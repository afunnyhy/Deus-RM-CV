...
class ArmorDetector:  # 模型推理类
    ...
    def detect_armor(self, orig_img, detect_color=None):
        ...
        # iou 默认为 0.7，如果发现同一个装甲板被框了多次，可以适当调低 iou (如 0.45 或 0.5)
        results = self.model(
            orig_img,
            imgsz=self.resize_shape,
            device="cuda:0" if self.CUDA else "cpu",
            verbose=False,
            stream=True,
            conf=self.min_confidence,  # 直接在这里过滤置信度
            iou=0.5  # 控制非极大值抑制的重叠阈值
        )
        ...
        for result in results:
            ...
            for i in range(len(boxes)):
                ...
                if is_show_video:
                    ...
                    cv2.putText(out_img, f"{label_name} {confidence:.2f}", (x1, y1 - 10),
